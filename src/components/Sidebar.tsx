...
import { IconShieldCheck, IconLayoutDashboard, IconMail, IconPencil, IconCalendar, IconGridDots, IconSettings, IconChartBar, IconBrain, IconCheckbox, IconBolt } from "@tabler/icons-react";
    ...
    <div className="w-36 shrink-0 bg-[#151A21] border border-[#242B35] rounded-xl p-3 h-fit relative z-50 pointer-events-auto">
      ...
      <nav className="flex flex-col gap-1">
        {tabs.map((tab) => {
          ...
          return (
            <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-2.5 py-2 rounded-lg text-xs font-medium w-full text-left transition-colors cursor-pointer border-0 outline-none relative z-50 pointer-events-auto ${isActive ? "bg-[rgba(74,143,194,0.16)] text-[#4A8FC2] font-semibold" : "bg-transparent text-[#9AA4B2] hover:bg-[#181E27] hover:text-[#F0F4F8]"}`}>
