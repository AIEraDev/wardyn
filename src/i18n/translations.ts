 ...
 * Import useQueueStore lazily via a module-level reference set from outside
 * to avoid a circular import while still being reactive to language changes.
 *
 * Components should use useTranslation() from this file instead of pulling
 * `t` out of the store, so they re-render when language changes.
 */
// Populated by the store's module on first load — avoids circular ESM issues.
let _useQueueStore: ((selector: (s: any) => any) => any) | null = null;
export function _registerUseQueueStore(
  hook: (selector: (s: any) => any) => any,
) {
  _useQueueStore = hook;
}
...
export const useTranslation = (): TranslationDictionary => {
  if (!_useQueueStore) {
    // Fallback before store is registered (e.g. SSR / tests)
    return TRANSLATIONS.en;
  }
  const language: SupportedLanguage = _useQueueStore(
